...
-- Add 'pending_signature' to document_status enum
ALTER TYPE public.document_status ADD VALUE IF NOT EXISTS 'pending_signature';

-- Create function to auto-transition document status based on responsibilities
...
BEGIN
  ...
  -- Auto-transition: pending_signature -> approved is NOT automatic
  -- (The user explicitly requested that a responsible person must approve it)

  RETURN NEW;
END;
$$;

-- Create trigger on document_responsibilities
