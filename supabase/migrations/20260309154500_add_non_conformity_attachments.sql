-- Attachments for Non Conformities
...
CREATE INDEX IF NOT EXISTS idx_non_conformity_attachments_nc_id
  ON public.non_conformity_attachments(non_conformity_id);
...
CREATE POLICY "View non conformity attachments" ON public.non_conformity_attachments
  FOR SELECT USING (
    non_conformity_id IN (
      SELECT id FROM public.non_conformities
